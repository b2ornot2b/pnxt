...
> Last updated: 2026-04-19 (Phase 8 Sprint 20 complete — M9 Bridge-Grammar Retry Telemetry delivered)

---

## Current State

**Phase 8 ("Operational Maturity") continues.** Sprint 20 ("Bridge-Grammar Retry Telemetry") opens **Milestone M9 ("Type-System Decision Data")** by instrumenting the bridge-grammar retry loop so that the Hindley-Milner extension decision described in `docs/research/lambda-type-system.md` §7 can be made from empirical data rather than a prior. Delivered: **`RetryEvent` wire format** (`src/types/bridge-telemetry.ts` — flat shape: `timestamp`, `attemptNumber`, `rejectionReason`, `errorCategory`, `promptHash` 16-hex, `responseExcerpt` ≤ 200 chars), **`TelemetryCategory` five-bucket taxonomy** (`schema_violation` / `type_mismatch` / `semantic_error` / `ifc_violation` / `other`, directly mapped to §7 buckets (a)-(d)), **`categorize()` pure function** (`src/bridge-grammar/retry-categorizer.ts` — reads already-structured `BridgeError.category`; priority: IFC on a `label`-bearing path wins over type-mismatch wins over schema-violation wins over semantic-error wins over other), **`RetryTelemetryCollector` + `InMemorySink` + `JsonlFileSink`** (`src/bridge-grammar/retry-telemetry.ts` — SHA-256-truncated `promptHash`, hard-capped `responseExcerpt`, sink failures swallowed with `console.warn` so telemetry never aborts generation; the JSONL sink does O(1) `fs.appendFile` per event, avoiding the O(n²) load-and-rewrite pathology of reusing `FileStorageBackend` directly), **instrumented `generateVPIRGraph`** (optional `collector?: RetryTelemetryCollector` on `VPIRGeneratorOptions`; both failure branches — "no tool_use block" and "validation failed" — now emit one categorised `RetryEvent` when a collector is injected, behaviour unchanged when absent for full backward compatibility), **`scripts/analyze-retries.ts` CLI** (category histogram + top-N rejection reasons; `--log`/`--top` flags; missing-log exit 1, parse errors exit 2), **`test-fixtures/sample-telemetry.jsonl`** (10-event fixture covering all five categories for CLI smoke tests), and **29 new tests across two files** (`retry-categorizer.test.ts` — 13 single-error arms + 3 priority-mixing cases + 20-sample hand-labeled accuracy run pinned at 20/20 = 100%, well above the 18/20 spec target; `retry-telemetry.test.ts` — privacy invariants, sink failure isolation, JSONL round-trip, and two generator-integration cases asserting one event per failed retry with `errorCategory: 'other'` for the no-`tool_use` branch). Total: **21 formally verified Z3 properties**, 91 test suites, 1664 tests (+2 suites, +29 tests over Sprint 19). Advisory panel composite score target: **9.72/10** (from 9.7 baseline). Advisor alignment gains: Church (HM decision defers cleanly in the spirit of minimal-sufficient calculi — the instrumentation is the measurement, not the commitment), Sutskever (LLM retry failure modes now have a structured taxonomy so the "Bridge Grammar is the hardest problem" assertion can be falsified or confirmed from data), de Moura (Z3/SMT cost of HM's first-order quantifier logic remains unpaid until `type_mismatch` > 40% justifies it), Liskov (`TelemetrySink` is a proper substitution seam — `InMemorySink` and `JsonlFileSink` are interchangeable, tests use the in-memory variant, production uses the file-backed one). **M9 is complete as code-complete**; the follow-up triage (≥ 100 real events + advisory-panel histogram review + Sprint 21 scoping) is tracked separately per `docs/sprints/sprint-20-bridge-grammar-telemetry.md` §"Post-Sprint Milestone M9".

**Phase 8 ("Operational Maturity") continues.** Sprint 19 ("Visual Authoring MVV — Interactive VPIR Graph Viewer") opens **Milestone M8 ("Dual Representation")** by shipping the first human-visible surface for a generated VPIR graph. Delivered: **`VPIRGraphViewer` React island** (`website/src/components/VPIRGraphViewer.tsx` — Cytoscape.js + `cytoscape-dagre`; six node-type fills matching the Sprint 17 `VPIRNodeType` union — observation/inference/action/assertion/composition/human; five classification border colours including the **Sprint 18 `external` band**; trust-level 0-4 border widths 1-4 px; rectangle vs. diamond shape driven by the `verifiable` flag with 0.6 opacity on unverifiable nodes; pan/zoom via Cytoscape defaults; pre-computed layer positions consumed verbatim from `exportGraphToJSON` with no additional layout pass), **`VPIRNodeTooltip` hover component** (surfaces the IFC `classification` + `trustLevel` and `verifiable` badge plus debug `id` on `mouseover` of any node; dismissed on `mouseout`), **Playground Astro route** (`/pnxt/playground/viewer`; MDX page importing the committed fixture as a static JSON module and mounting the viewer as `client:load`; new sidebar group `Playground`), **Weather-benchmark fixture** (`website/src/fixtures/weather-benchmark.vpir.json` — 7 nodes / 7 edges; security-label timestamps normalised to a fixed epoch so the committed artefact is byte-stable across regenerations), **`scripts/export-vpir-fixtures.ts` CLI** (`buildWeatherFixture` + `writeWeatherFixture` helpers; guards against silent zero-node exports; runnable via `npx tsx scripts/export-vpir-fixtures.ts`), **`@astrojs/react` integration** (registered in `website/astro.config.mjs`; React 18 + `cytoscape` + `cytoscape-dagre` added as website dependencies; `@types/react`, `@types/cytoscape`, `@playwright/test` added as devDependencies), **Jest roots extension** (`scripts/` added alongside `src/`; dedicated `tsconfig.jest.json` relaxes `rootDir` so ts-jest picks up the scripts workspace), and **Playwright E2E smoke spec** (`website/tests/viewer.spec.ts` + `website/playwright.config.ts`; asserts the viewer heading, a Cytoscape `<canvas>` element, and zero console errors on `/pnxt/playground/viewer`; runs manually via `npm run test:e2e` in `website/` — deliberately not wired into CI to avoid browser-binary install cost). Bidirectional editing, the pre-execution review gate, and dense-code surface syntax remain deferred. Total: **21 formally verified Z3 properties**, 89 test suites, 1635 tests (+2 suites, +23 tests over Sprint 18). Advisory panel composite score target: **9.7/10** (from 9.65 baseline). Advisor alignment gains: Kay (first human-visible surface for the dual-representation pillar), Liskov (stable typed `VPIRGraphJSON` contract is now the viewer's wire format — no new coupling), Myers (IFC classification + trust level + verifiable flag are first-class visual attributes, not buried in JSON), Sutskever (LLM-generated graphs are now directly human-inspectable, closing the black-box gap between generation and execution).

**Phase 8 ("Operational Maturity") continues.** Sprint 18 ("Node Catalog Expansion — `llm-inference` + Descriptor Metadata") delivered the **`ToolUIMetadata` type + optional `uiMetadata` field on `ToolRegistration`** (`displayName`/`category`/`icon`/`tags`/`examples`; backward compatible — all existing callers still compile with zero changes), the **new `'llm_call'` `SideEffect` variant** (mapped to `requiredTrustLevel: 2` in `SIDE_EFFECT_TRUST_REQUIREMENTS`), the **`'external'` IFC classification band** (sits above `restricted` in `CLASSIFICATION_ORDER`; added to the bridge-grammar JSON Schema enum and `VALID_CLASSIFICATIONS` validator), the **`llm-inference` handler** (9th standard handler; Anthropic SDK dispatch via a module-level `setLLMInferenceClientFactory` test seam so CI can stub the client without touching `@anthropic-ai/sdk`; input `{prompt, model?, maxTokens?, systemPrompt?}` → output `{response, tokensUsed, model}` with default model `claude-sonnet-4-20250514` and default `maxTokens: 1024`; `costCategory: 'expensive'`, `timeout: 60_000`, `retryable: true`, `idempotent: false`, `requiredTrustLevel: 2`), **`uiMetadata` backfill on all 9 handlers** (categories: `IO` — http-fetch, file-read, file-write; `Data` — json-transform, data-validate; `Compute` — string-format, math-eval, unit-convert; `AI` — llm-inference), **`ToolRegistry.getManifest()` + `ToolManifestEntry` type** (merges registration core fields with `uiMetadata.category`/`tags`/`displayName`; fresh arrays on each call so catalog consumers cannot mutate the registry; ordering matches registration order), **ACI gateway `llm_call` label override** (result labels are forced to `{trustLevel: 1, classification: 'external'}` whenever `registration.sideEffects` includes `'llm_call'`, regardless of caller trust — Myers's noninterference rule centralised in `aci-gateway.ts:266-278`; non-llm handlers keep their trust-derived label), **`buildSystemPrompt(availableHandlers)` + `VPIR_SYSTEM_PROMPT_BASE` export** (`llm-vpir-generator.ts` — base prompt factored out; `generateVPIRGraph` now accepts an optional `registry: ToolRegistry` and injects an `Available action operations` stanza listing every registered handler name, closing the "blind catalog" bug where the LLM would hallucinate non-existent handler names), and the **Weather benchmark LLM summary extension** (`createWeatherVPIRGraphWithLLMSummary` — chains `build-summary-prompt` inference → `summarize-weather` action (invoking `llm-inference`) → rewired `assert-valid` off the LLM `response` port; runs in CI with the module-level client factory stub; end-to-end test pins that the gateway-forced `external` label is observed at the summary boundary and that the same label blocks a subsequent invocation of a public sink). Total: **21 formally verified Z3 properties**, 87 test suites, 1612 tests (+32 over Sprint 17). Advisory panel composite score target: **9.65/10** (from 9.6 baseline). Advisor alignment gains: Myers (LLM output labeling — noninterference proven at the gateway, not delegated to each handler), Sutskever (LLM is a first-class catalog entry + generator sees live manifest), Liskov (optional `uiMetadata` is zero-cost for non-UI callers and structurally typed for UI consumers), de Moura (`costCategory: 'expensive'` on `llm-inference` gives future Z3 cost-modelling a concrete subject), Kay (first AI-category handler — pipelines can now express LLM-in-the-loop natively).

**Phase 8 ("Operational Maturity") continues.** Sprint 17 ("Human-in-the-Loop Primitive") delivered the **`'human'` VPIR node type** (extends `VPIRNodeType` with a `humanPromptSpec` for message/timeout/explicit-provenance requirements; validator enforces `verifiable: false` and presence of `humanPromptSpec`), **`HumanGateway` interface + `CLIHumanGateway` + `NoopHumanGateway`** (pluggable delivery seam at `src/vpir/human-gateway.ts`, with stdin/stdout reference implementation and a test double that records call counts), **`executeHuman()` in the interpreter** (gateway guard → capability guard on `human.attention` → provenance input-join over predecessor labels → pre-await journal checkpoint for crash recovery → `gateway.prompt()` → `responseLabel = joinLabels(humanLabel, inputJoin)` → emit `AuditEvent` with `actor.type: 'human'` via `humanAuditSink` → return response value), **`human-approval` NL protocol** (5-row transition table: `initiated → awaiting_human → completed | rejected | timed_out | awaiting_human(self)`; refactored `TERMINAL_STATES` to a per-protocol map so `rejected`/`timed_out` are terminal only for human-approval, preserving the three existing protocols), **`human.attention` capability** (registered via `registerHumanAttentionCapability`; default `requiredTrustLevel: 3`; denied to low-trust agents; negotiation captured in the ACI audit trail), **Z3 verifier uninterpretable markers** (human nodes reported as `status: 'uninterpretable'` with `reason: 'human-node'` and `affectedNodes`; overall `verified` flag unchanged on machine-only graphs; graphs with human nodes remain verified on the machine subgraph — all 21 Z3 properties preserved), and the **Weather benchmark operator-approval gate** (`createWeatherVPIRGraphWithApproval` inserts an `approve-fetch` human node before the outbound action, routed through a `verify-approval` inference gate that fails the pipeline if the operator rejects; runs in CI with `NoopHumanGateway({ response: 'approved' })`, swaps in `CLIHumanGateway` for interactive development). **M6 (Human-in-the-Loop) is now complete.** The first `AuditEvent` with `actor.type: 'human'` is now emitted from `src/`, closing the runtime-footprint gap identified in `docs/research/hitl-primitive.md` (status flipped to Accepted and Implemented). Total: **21 formally verified Z3 properties**, 88 test suites, 1580+ tests. Advisory panel composite score target: **9.6/10** (from 9.55 baseline). Advisor alignment gains: Liskov (well-typed node type), Myers (provenance-join prevents laundering), Agha (HITL layered above DPN — bisimulation preserved), Pearl (human response causally linked to input lineage).

**Phase 8 ("Operational Maturity") has begun.** Sprint 16 ("Durable VPIR Execution") delivered **VPIRJournal** (crash-safe persistence substrate with `JournalEntry` + `JournalCheckpoint` records, in-memory and file-backed implementations, SHA-256 graph content hashing for structural integrity validation, and `JOURNAL_SCHEMA_VERSION` guard rejecting stale-build entries), **executeGraph durability integration** (optional `journal` + `resumeFrom` options on `VPIRExecutionOptions`; both sequential and wave-based parallel paths journal every settled node and emit per-node checkpoints; `SecurityLabel` carried verbatim through replay so `checkIFCFlow` produces identical results), **`resumeFromCheckpoint`** (reconstructs `ExecutionState` from the latest checkpoint, validates graph content hash, rejects structurally-changed graphs with `JournalGraphHashError`), **Channel + Process snapshot/restore interfaces** (interface-only contracts with FIFO-preserving buffer snapshots, `bufferSize`/`processId` validation, and `ChannelSnapshotMismatchError`/`ProcessSnapshotMismatchError`), and **Weather-benchmark kill-and-restart scenario** (7-node graph; bomb-handler crashes mid-graph; fresh `FileBackedJournal` simulates process restart; resumed run produces outputs deep-equal to an uninterrupted reference; journal file stays below the ADR-001 10 KB budget). **M5 (Crash-Safe Execution) is now complete.** Total: **21 formally verified Z3 properties**, 86 test suites, 1530+ tests. Advisory panel composite score target: **9.55/10** (from 9.5 baseline). ADR-001 promoted from Proposed to Accepted with full Implementation Notes covering schema versioning, hash canonicalisation, per-node checkpoint cadence, parallel-path journaling, SIGKILL equivalence argument, and the documented follow-on to replace R-M-W JSON with newline-delimited append-only writes.

**Phase 7 ("Self-Hosting Paradigm") is complete.** Sprint 15 ("Verified Self-Modification + Research Frontier") delivered **Causal Impact Analyzer** (builds causal graphs from VPIR dependencies, traces downstream effects of modifications, estimates risk with 4-factor scoring, suggests mitigations and Active Inference risk reduction patches), **Modification Confidence Scorer** (5-dimensional scoring: structural safety, property preservation, IFC compliance, causal impact, rollback feasibility — with auto-approve/require-review/reject thresholds), **Self-Modification Orchestrator** (production-grade pipeline: propose → evaluate → apply with Z3 property preservation, causal analysis, confidence scoring, and transactional commit/rollback), **Real Self-Modification Scenarios** (5 production-quality scenarios exercising full orchestrator: add caching, add confidence gate, modify trust levels with IFC violation detection, add parallel verification, remove redundant stage), and **Phase 7 Comprehensive Evaluation** (capstone demonstrating M2+M3+M4 together with advisory panel alignment metrics and 6 formalized research contributions). **M4 (Self-Modification) is now complete.** Total: **21 formally verified Z3 properties**, 83 test suites, 1485+ tests. Advisory panel composite score: **9.5/10** (from 9.45 baseline). All Phase 7 milestones (M2, M3, M4) are complete.

Previously completed Sprint 14 ("VPIR Diff/Patch + Self-Mutation") delivered **VPIR Diff Engine** (structured graph comparison with node/edge operations, diff inversion for rollback, and sequential composition), **VPIR Patch Engine** (atomic all-or-nothing patch application with conflict detection and structural validation), **Graph Transaction Manager** (begin → patch → validate → verify → commit/rollback semantics with full execution trace), **Z3 Property Preservation Verifier** (two-strategy verification: HoTT transport for unaffected properties, Z3 re-verification for affected — classifies diff impact to minimize solver calls), **Mutable Self-Description** (extends Sprint 9's self-hosting PoC with verified pipeline modifications: add/remove/modify stages with IFC-checked edge rerouting), and **Self-Mutation Benchmark** (6 scenarios testing add-stage, remove-stage, trust modification, parallel branches, IFC violations, and operation changes). **M4 foundation complete.**

Previously completed Sprint 13 ("Autonomous LLM Pipeline") delivered **Iterative Refinement Generator** (multi-attempt VPIR generation with structured diagnosis feedback and two feedback strategies: structured and contextual), **Neurosymbolic Pipeline Integration** (P-ASP confidence scoring and Active Inference graph patching connected into the Bridge Grammar generation pipeline with oscillation detection and patch history tracking), **Autonomous Pipeline Orchestrator** (end-to-end NL → VPIR → Z3 → HoTT → DPN → Result pipeline with 5 configurable stages: generate, refine, verify, categorize, execute), and **Autonomous Pipeline Benchmark** (7 end-to-end scenarios exercising different handler combinations measuring generation success rate, confidence, and pipeline success). **M3 (LLM-Native Programming) is now complete.** Total: **21 formally verified Z3 properties**, 72 test suites, 1286+ tests. Advisory panel composite score: **9.4/10** (from 9.35 baseline). Phase 7 transitions pnxt from verified prototype to self-modifying, LLM-programmable system.

Previously completed Sprint 12 ("Reliable Bridge Grammar + Error Recovery"): **Bridge Grammar Error Taxonomy** (typed error hierarchy with 6 categories, repair hints, and structured LLM feedback), **Auto-Repair Engine** (automatic correction of truncated JSON, missing fields, wrong enums, duplicate IDs, and missing roots/terminals), **Confidence Scorer** (P-ASP-inspired 4-dimensional scoring: structural, semantic, handler coverage, topological), **Z3 Graph Pre-Verification** (4 formal properties: acyclicity, input completeness, IFC monotonicity, handler trust), **Reliable Generation Pipeline** (multi-stage orchestration: generate → diagnose → repair → re-validate → score → verify), and **Error Recovery Benchmark** (7 scenarios covering all error categories).
...
| Phase 8 Sprint 18 | 87 | 1612 | ~29,500 |
